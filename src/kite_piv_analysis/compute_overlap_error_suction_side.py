...
def analyze_suction_side_overlap(q=0.5):
    ...
    for aoa in [23]:  # Focus on aoa=13 as primary case
...
def compute_sigmoid_stitching_values(u_left, u_right):
    """Return stitched u-values and sigmoid weights for overlapping regions."""
    mask = ~np.isnan(u_left) & ~np.isnan(u_right)
    stitched = np.full_like(u_left, np.nan)
    w_left = np.full_like(u_left, np.nan)
    w_right = np.full_like(u_left, np.nan)

    for row in range(mask.shape[0]):
        cols = np.where(mask[row])[0]
        if cols.size == 0:
            continue
        s = np.linspace(0, 1, cols.size)
        sigmoid = 1.0 / (1.0 + np.exp(-10 * (s - 0.5)))
        w_right_vals = sigmoid
        w_left_vals = 1.0 - sigmoid
        w_left[row, cols] = w_left_vals
        w_right[row, cols] = w_right_vals
        stitched[row, cols] = (
            w_left_vals * u_left[row, cols] + w_right_vals * u_right[row, cols]
        )

    return stitched, w_left, w_right


def plot_y1_weighted_overlap(y1_data, output_dir, q=0.5):
    """
    Plot the delta between the raw plane values and the stitched smoothing result.
    Layout is identical to plot_y1_overlap_differences for easy comparison.
    """
    if y1_data is None:
        print("No Y1 data available for plotting weighted differences")
        return
    ...
    if not has_x1_x2 and not has_x2_x3:
        print("Missing plane data for Y1 weighted plot")
        return
    ...
    if has_x1_x2:
        u1 = interpolated["X1"]["u"]
        u2 = interpolated["X2"]["u"]
        stitched, _, _ = compute_sigmoid_stitching_values(u1, u2)
        diff = np.full_like(stitched, np.nan)
        mask = ~np.isnan(stitched) & ~np.isnan(u1)
        diff[mask] = u1[mask] - stitched[mask]
        diffs["X1-X2"] = diff
    if has_x2_x3:
        u2 = interpolated["X2"]["u"]
        u3 = interpolated["X3"]["u"]
        stitched, _, _ = compute_sigmoid_stitching_values(u2, u3)
        diff = np.full_like(stitched, np.nan)
        mask = ~np.isnan(stitched) & ~np.isnan(u2)
        diff[mask] = stitched[mask] - u2[mask]
        diffs["X2-X3"] = diff

    all_diff_values = []
    for diff in diffs.values():
        all_diff_values.extend(diff[~np.isnan(diff)])

    if all_diff_values:
        vmax = max(np.percentile(np.abs(all_diff_values), 95), 1.0)
    else:
        vmax = 5.0
    ...
    if "X1" in interpolated:
        valid = ~np.isnan(interpolated["X1"]["u"])
        ax.pcolormesh(
            grid_x,
            grid_y,
    ...
    if "X2" in interpolated:
        valid = ~np.isnan(interpolated["X2"]["u"])
        ax.pcolormesh(
            grid_x,
            grid_y,
            np.where(valid, 1.0, np.nan),
    ...
    if "X3" in interpolated:
        valid = ~np.isnan(interpolated["X3"]["u"])
        ax.pcolormesh(
            grid_x,
            grid_y,
            np.where(valid, 1.0, np.nan),
    ...
    ax.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"upper/lower split (y = {y_threshold:.3f} m)",
    )

    upper_vals = []
    lower_vals = []
    for diff in diffs.values():
        upper_diff = diff[grid_y >= y_threshold]
        lower_diff = diff[grid_y < y_threshold]
        upper_vals.extend(upper_diff[~np.isnan(upper_diff)])
        lower_vals.extend(lower_diff[~np.isnan(lower_diff)])

    def stats_text(values, label):
        if not values:
            return f"{label}: no valid points"
        bias = np.mean(values)
        scatter = np.std(values)
        mean_abs = np.mean(np.abs(values))
        n = len(values)
        return (
            f"{label}: Bias {bias/U_inf*100:+.1f}% U∞ | "
            f"Scatter {scatter/U_inf*100:.1f}% U∞ | "
            f"|Δu| {mean_abs/U_inf*100:.1f}% U∞ | N={n}"
        )

    bbox_props = dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.85)
    ax.text(
        0.02,
        0.98,
        stats_text(upper_vals, "Upper (far)"),
        transform=ax.transAxes,
        fontsize=10,
        verticalalignment="top",
        horizontalalignment="left",
    ...
    ax.text(
        0.02,
        0.02,
        stats_text(lower_vals, "Lower (near)"),
        transform=ax.transAxes,
        fontsize=10,
        verticalalignment="bottom",
        horizontalalignment="left",
    ...
    if im is not None:
        plt.colorbar(im, ax=ax, label=r"$\Delta u_{\mathrm{stitched}}$ (m/s)")

    plt.title(
        f"Y1 Suction Side: Sigmoid-stitched differences (X1 vs stitching, stitching vs X2)\n"
        f"Upper/lower split at {q:.0%} of flipped y-range",
        fontsize=12,
        fontweight="bold",
    )
    plt.xlabel("x (m)")
    plt.ylabel("y (m) [flipped]")
    plt.tight_layout()

    save_path = output_dir / "overlap_Y1_suction_weighted.png"
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    print(f"\nWeighted plot saved to: {save_path}")
...
def main():
    ...
    if y1_data is not None:
        ...
        plot_y1_weighted_overlap(y1_data, output_dir, q=q)
